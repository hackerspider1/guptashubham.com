...
import {
  ...
  ClockIcon, MonitorIcon, XIcon, ComputerIcon, MessageSquare, Trophy
} from 'lucide-react';
...
const AnimatedCounter = ({ target, text, icon: Icon }: AnimatedCounterProps) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center">
      {Icon && <Icon className="mb-2 h-6 w-6 text-[#FD2]" />}
      <div ref={ref} className="text-2xl font-bold">{isVisible ? count : 0}</div>
      {text && <div className="mt-1 text-sm text-muted-foreground">{text}</div>}
    </div>
  );
};
...
const HomeClientPage = () => {
  ...
  // Stats for home page counter
  const stats = [
    { count: 600, text: "Hall of Fame", icon: Trophy },
    { count: 1337, text: "Bugs Found", icon: BugIcon },
    { count: 82, text: "Projects", icon: Briefcase }
  ];
  ...
  const [isLoaded, setIsLoaded] = useState(() => {
    if (typeof window !== 'undefined') {
      return localStorage.getItem('pageLoaded') === 'true';
    }
    return false;
  });
  ...
  return (
    <>
      <main className="min-h-screen bg-white">
        ...
        <section
          id="hero"
          className="relative h-auto min-h-[70vh] w-full overflow-hidden md:h-[90vh]"
        >
          <div className="relative flex h-full w-full flex-col">
            <div className="absolute left-0 top-0 z-0 overflow-hidden sm:h-[750px] h-[580px] w-full">
              <AnimatedBackground />
            </div>
            <div className="container relative z-10 flex h-full flex-col justify-center py-10 md:py-20">
              <div className="mx-auto mb-8 flex max-w-[620px] flex-col gap-8 text-center md:mb-12">
                <h1 className="text-2xl font-bold text-foreground md:text-4xl lg:text-5xl">
                  <span className="text-2xl font-light text-foreground md:text-4xl">
                    Passionate
                  </span>{" "}
                  <span className="z-10 inline-block">
                    Security Researcher
                    <div className="relative -z-10 h-1.5 w-full max-w-full rounded-full bg-yellow-300/70 blur-[2px]" />
                  </span>
                </h1>
                <p className="text-base text-muted-foreground md:text-lg">
                  A talented bug hunter with a keen eye for vulnerabilities and a
                  passion for penetration testing. I've been helping secure the web
                  for over 10 years.
                </p>
                <div className="flex flex-wrap justify-center gap-4">
                  <Link
                    href="/#contact"
                    className="inline-flex items-center gap-2 rounded-md bg-[#FD2]
                      px-4 py-3 text-sm font-semibold text-black transition-all hover:bg-[#fd2]/90 md:px-6 md:py-3 md:text-base"
                  >
                    <MessageSquare size={18} /> Let's talk
                  </Link>
                  <Link
                    href="/achievements"
                    className="inline-flex items-center gap-2 rounded-md border border-[#FD2]
                      px-4 py-3 text-sm font-semibold text-foreground transition-all hover:bg-[#fd2]/10 md:px-6 md:py-3 md:text-base"
                  >
                    <Trophy size={18} className="text-[#FD2]" /> Achievements
                  </Link>
                </div>
              </div>

              <div className="relative mx-auto h-[160px] w-[220px] sm:h-[200px] sm:w-[280px] md:h-[200px] md:w-[320px]">
                <div className="animate-in fade-in absolute bottom-0 left-1/2 -translate-x-1/2 opacity-0 [--duration:500ms] [--delay:300ms]">
                  <HeroLogos logos={logos} />
                </div>
              </div>

              <div className="mt-6 flex flex-col items-center justify-center gap-6 md:mt-12 md:flex-row">
                {stats.map((stat, i) => (
                  <GlassCard key={i} className="">
                    <AnimatedCounter
                      target={stat.count}
                      text={stat.text}
                      icon={stat.icon}
        ...
        <div className="relative min-h-screen w-full flex flex-col items-center justify-center bg-black overflow-hidden text-white">
          ...
          <div className="bg-gradient-to-b from-black to-zinc-900 w-full flex justify-center pb-4 sm:pb-8">
            <div className="relative z-10 max-w-6xl w-full px-4 sm:px-6">
              <div className="text-center mb-8 sm:mb-16 relative sm:h-[80vh] md:h-[90vh] h-auto min-h-[70vh] flex flex-col justify-center items-center py-6 sm:py-0">
                <div className="sm:max-w-lg w-full relative flex flex-col justify-center items-center">
                  <div className="relative w-full h-[180px] sm:h-[220px] md:h-[250px]">
                    <HeroLogos logos={[...logos]} />
                  </div>
                  <img 
                    className="w-[220px] sm:w-[280px] md:w-[320px] -mt-2 sm:mt-0" 
                    src="/shubham_gupta.png" 
                    alt="Shubham Gupta" 
                  />
                  <h1 className="relative text-3xl sm:text-4xl md:text-5xl font-bold tracking-tight mb-2 sm:mb-4 text-white">
                    Shubham Gupta
                  </h1>
                  <div className="text-sm sm:text-base md:text-lg text-gray-300 mb-4 sm:mb-8">
